...
// Modified: March 24, 2016
	...
	class url {
		...
		public static function validate($slug, $value, $field, $options) {
			...
			$pattern = '^http(s)?://[a-z0-9-_.]+\.[a-z]{2,4}';
			...
			if (empty($match) ) {
				...
				if (empty($value) && empty($field['required'])) {
					...
					$result['feedback'] = sprintf(__('The submitted information is not a valid URL, please check the %s field.', 'theme translation'), '<strong>'.strtolower($field['name'].'</strong>'));
				}
			}
			else {
